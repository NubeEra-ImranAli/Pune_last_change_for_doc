...
def deep_learning(data):
    ...
    for name in data['NAME'].unique():
        ...
        # Handle missing values in X_train_shap for SHAP calculation
        ...
        shap_values = explainer(X_test_shap) # Use X_test_shap for SHAP values

        shap.plots.bar(shap_values, show=False)
        plt.title(f'SHAP Feature Importance - {name}')
        plt.tight_layout()
        plt.savefig(f'shap_{name}.png')  # Save SHAP plot
        plt.close()

        # Select top 6 features based on SHAP mean abs value
        shap_importance = np.abs(shap_values.values).mean(axis=0)
        important_indices = np.argsort(shap_importance)[::-1][:6]
        important_features = [X.columns[i] for i in important_indices]

        # === Handle missing values in X with selected important features ===
        ...
        # === Rebuild Poly + Scale with selected features ===
        ...
        mae = mean_absolute_error(y_test, ensemble_preds) # Calculate MAE

        print(f"{name} - RMSE: {np.sqrt(mse):.3f}, R²: {r2:.4f}, MAE: {mae:.3f}") # Print MAE
        total_mse += mse
        total_r2 += r2
        total_mae += mae # Accumulate total MAE
        count += 1
    ...
    mae_avg = total_mae / count # Calculate average MAE

    print(f"\n⏱ Time: {end_time - start_time:.2f}s")
    print(f"📊 Avg RMSE: {rmse_total:.3f}")
    print(f"📈 Avg R²: {r2_avg:.4f}")
    print(f"📉 Avg MAE: {mae_avg:.3f}") # Print average MAE
    return f"{rmse_total:.3f}", f"{r2_avg:.4f}", f"{mae_avg:.3f}", f"{end_time - start_time:.2f}"  # Return average MAE
